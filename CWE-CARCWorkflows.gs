...
function buildWorkflowHTMLDark(issue, color) {
    // ── Route to CARC-specific workflow if code is present ──────────────
    if (issue.carcCode && String(issue.carcCode).trim() !== '') {
    return buildCARCWorkflow(issue, color);
  }

  // ── Fallback: root cause workflow (no CARC code on claim) ───────────
